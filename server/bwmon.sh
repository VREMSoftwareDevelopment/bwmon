...
[ -z "${1}" ] && display && exit 1
[ -z "${2}" ] && display && exit 1
bc -version > /dev/null 2>&1 || { display; exit 1; }
...
create() {
  ...
  if ! iptables -L FORWARD --line-numbers -n | grep "RRDIPT" | grep "1" > /dev/null; then
    if iptables -L FORWARD -n | grep "RRDIPT" > /dev/null; then
  ...
  grep "${LAN0_IFACE}" /proc/net/arp | while read -r IP _TYPE _FLAGS MAC _MASK _IFACE
  do
    # Add iptable rules (if non existing).
    if ! iptables -nL RRDIPT | grep "${IP} " > /dev/null; then
      iptables -I RRDIPT -d "${IP}" -j RETURN
      iptables -I RRDIPT -s "${IP}" -j RETURN
    fi
  done
  # LAN1 For each host in the ARP table
  grep "${LAN1_IFACE}" /proc/net/arp | while read -r IP _TYPE _FLAGS MAC _MASK _IFACE
  do
    # Add iptable rules (if non existing).
    if ! iptables -nL RRDIPT | grep "${IP} " > /dev/null; then
      iptables -I RRDIPT -d "${IP}" -j RETURN
