...
type NullableValue = { value: number | null };
...
describe('SortUtils', () => {
    describe('comparator', () => {
        ...
        const data: Person[] = [alice, bob, { name: 'Charlie', age: 35 }, { name: 'Eve', age: undefined }, { name: 'Mallory' }];

        it('ascending 1', () => {
            const a = { value: 20 };
            const b = { value: 10 };
            const fn = comparator<Value>(true, 'value');
        ...
        it('descending 1', () => {
            const a = { value: 10 };
            const b = { value: 20 };
            const fn = comparator<Value>(false, 'value');
            expect(fn(a, b)).toEqual(1);
        });
        ...
        it('should sort ascending by age', () => {
            const cmp = comparator<Person>(true, 'age');
        ...
        it('should return 0 for empty string key', () => {
            const a = { value: 1 };
            const b = { value: 2 };
            const fn = comparator<UnknownRecord>(true, '');
        ...
        it('should return 0 for key with special characters', () => {
            const a = { value: 1 };
            const b = { value: 2 };
            const fn = comparator<UnknownRecord>(true, '$invalid!');
            expect(fn(a, b)).toBe(0);
        });
        ...
        it('should return 0 for symbol key', () => {
            const a = { value: 1 };
            const b = { value: 2 };
            const fn = comparator<UnknownRecord>(true, Symbol('value') as unknown as string);
            expect(fn(a, b)).toBe(0);
        });
        ...
        it('should handle null values for orderBy', () => {
            const a = { value: null };
            const b = { value: 2 };
            const fn = comparator<NullableValue>(true, 'value');
        ...
        it('should handle non-object arguments', () => {
            const fn = comparator<UnknownRecord>(true, 'value');
            expect(fn(42 as unknown as UnknownRecord, { value: 1 })).toBe(0);
            expect(fn({ value: 1 }, 42 as unknown as UnknownRecord)).toBe(0);
        });
    ...
    describe('sort', () => {
        ...
        it('should sort using comparator', () => {
            const arr = [{ value: 3 }, { value: 1 }, { value: 2 }];
            const cmp = comparator<Value>(true, 'value');
            const sorted = sort(arr, cmp);
