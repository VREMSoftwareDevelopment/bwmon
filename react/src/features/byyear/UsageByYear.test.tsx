...
describe('UsageByYear', () => {
    ...
    beforeEach(() => {
        vi.mocked(useUsageByYear).mockReturnValue({ data, loading: false, error: null });
        vi.mocked(useSortDesc).mockReturnValue({
            ascending: true,
            setAscending: vi.fn(),
            orderBy: 'id',
            setOrderBy: vi.fn(),
    ...
    it('renders loading state', () => {
        vi.mocked(useUsageByYear).mockReturnValue({ data: undefined, loading: true, error: null });
        renderComponent();
    ...
    it('renders error state', () => {
        vi.mocked(useUsageByYear).mockReturnValue({ data: undefined, loading: false, error: 'Failed to load data' });
        renderComponent();
    ...
    it('handles rows per page change', () => {
        ...
        expect(usePagination(20).setRowsPerPage).toHaveBeenCalledWith(29);
    });

    it('handles sort request by year', () => {
        renderComponent();
        fireEvent.click(screen.getByText('Year'));
        expect(useSortDesc('id').setAscending).toHaveBeenCalled();
        expect(useSortDesc('id').setOrderBy).toHaveBeenCalledWith('id');
    });
    ...
    it('handles sort request by total', () => {
        ...
        expect(useSortDesc('id').setAscending).toHaveBeenCalled();
        expect(useSortDesc('id').setOrderBy).toHaveBeenCalledWith('total');
    });
