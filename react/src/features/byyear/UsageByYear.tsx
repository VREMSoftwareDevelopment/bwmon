...
const yearCell = new CellInfo<UsageSummary>('id', true, 'left', 'Year', false);

const cellInfos: CellInfo<UsageSummary>[] = [
    yearCell,
    new CellInfo<UsageSummary>('download', false, 'right', 'Down', true, usageInGBytes),
    new CellInfo<UsageSummary>('upload', false, 'right', 'Up', true, usageInGBytes),
    new CellInfo<UsageSummary>('total', true, 'right', 'Total', true, usageInGBytes),
    new CellInfo<UsageSummary>('average', false, 'right', 'Average', true, usageInGBytes),
    new CellInfo<UsageSummary>('days', false, 'left', 'Days', true),
];
...
const UsageByYear = () => {
    ...
    const { ascending, setAscending, orderBy, setOrderBy } = useSortDesc(yearCell.id);
    ...
    const sortedData = useMemo(
        () => (data ? sort(data, comparator<UsageSummary>(ascending, orderBy)) : []),
