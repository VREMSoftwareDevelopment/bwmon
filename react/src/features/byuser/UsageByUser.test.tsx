...
describe('UsageByUser', () => {
    beforeEach(() => {
        vi.mocked(useUsageByUser).mockReturnValue(hookState);
        vi.mocked(useSortAsc).mockReturnValue({
            ascending: true,
            setAscending: vi.fn(),
            orderBy: 'IP',
            setOrderBy: vi.fn(),
    ...
    it('renders loading state', () => {
        vi.mocked(useUsageByUser).mockReturnValue({ ...hookState, data: undefined, loading: true });
        renderComponent();
    ...
    it('renders error state', () => {
        vi.mocked(useUsageByUser).mockReturnValue({
            ...hookState,
            data: undefined,
            loading: false,
    ...
    it('handles next page change', () => {
        ...
        expect(usePagination(20).setPage).toHaveBeenCalledWith(1);
    });
    ...
    it('handles rows per page change', () => {
        ...
        expect(usePagination(20).setRowsPerPage).toHaveBeenCalledWith(27);
    });

    it('handles sort by IP', () => {
        renderComponent();
        fireEvent.click(screen.getByText('IP'));
        expect(useSortAsc('IP').setAscending).toHaveBeenCalled();
        expect(useSortAsc('IP').setOrderBy).toHaveBeenCalledWith('IP');
    });
    ...
    it('handles sort by MAC', () => {
        ...
        expect(useSortAsc('IP').setAscending).toHaveBeenCalled();
        expect(useSortAsc('IP').setOrderBy).toHaveBeenCalledWith('MAC');
    });
    ...
    it('handles sort by USER', () => {
        ...
        expect(useSortAsc('IP').setAscending).toHaveBeenCalled();
        expect(useSortAsc('IP').setOrderBy).toHaveBeenCalledWith('user');
    });
    ...
    it('handles sort by Total', () => {
        ...
        expect(useSortAsc('IP').setAscending).toHaveBeenCalled();
        expect(useSortAsc('IP').setOrderBy).toHaveBeenCalledWith('total');
    });
    ...
    it('handles user filter change', () => {
        vi.mocked(Search).mockImplementation(({ onChange }) => <input data-testid="user-filter" onChange={onChange} />);
        renderComponent();
    ...
    it('returns no rows if data.usage is empty', () => {
        vi.mocked(useUsageByUser).mockReturnValue({ ...hookState, data: { ...data, usage: [] } });
        renderComponent();
    ...
    const data = {
        ...
        total: { id: 2, download: 10000000, upload: 1000000, total: 11000000, average: 100000, days: 10 },
    };
