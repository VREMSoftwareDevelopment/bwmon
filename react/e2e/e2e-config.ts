...
const getTBodyRowTexts = async (page: Page): Promise<string[]> => {
    const locator = page.locator('tbody tr');
    return await locator.evaluateAll((elements: HTMLElement[]) => elements.map((e) => e.innerText));
};

const getTHeadRowTexts = async (page: Page): Promise<string[]> => {
